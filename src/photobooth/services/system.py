...
class SystemService(BaseService):
    ...
    def _util_launchd_control_macos(self, state: Literal["start", "stop", "restart"]):
        ...
        try:
            ...
            # non zero returncode
            logger.warning(f"service {MACOS_LABEL_NAME} currently inactive, need to restart by yourself! error {exc}")
        except Exception as exc:
            logger.error(f"error: {exc}")

        else:
            # no error, service restart ok
            logger.info(f"service {MACOS_LABEL_NAME} currently active, restarting")
            try:
                if state == "stop":
